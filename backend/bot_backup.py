...
def init_db():
    with get_db() as db:
        # جدول المستخدمين - مع blocked و role
        db.execute("""
        ...
        """)
        try:
            db.execute("ALTER TABLE recharge_log ADD COLUMN pin TEXT")
        except: pass
        ...
        if not cur.fetchone():
            ...
            db.commit()
            print(f"✅ أدمن: {admin_user} / {admin_pass}")
...
def is_admin_user(user):
    admin_user = os.getenv("ADMIN_USERNAME", "admin")
    return user and (user["username"] == admin_user or user["role"] == "admin")
...
def api_report():
    ...
    try:
        log_row = None
        with get_db() as db:
            log_row = db.execute("SELECT * FROM recharge_log WHERE auth_id=?", (auth_id,)).fetchone()
        if log_row:
            status_text = "✅ نجح" if success else "❌ فشل"
            msg = (f"📱 شحن جديد من التطبيق\n"
                   f"👤 المستخدم ID: {log_row['user_id']}\n"
                   f"📦 المنتج: {log_row['product_id']}\n"
                   f"📱 المستلم: {log_row['receiver']}\n"
...
def api_create_user():
    ...
    try:
        ...
        return jsonify({"success": False, "message": "❌ اليوزر موجود"})
...
def api_block_user():
    ...
    with get_db() as db:
        ...
        if target:
            ...
            add_notification(db, target["id"], status, "تواصل مع الإدارة لمزيد من المعلومات")
        db.commit()
    return jsonify({"success": cur.rowcount > 0})
...
def main():
    ...
    print(f"✅ البوت شغال! API على البورت {API_PORT}")
    app.run_polling()
